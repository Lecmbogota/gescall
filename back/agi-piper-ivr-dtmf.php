...
function get_audio_chunk($chunk) {
    ...
    $data = json_encode(array("text" => $chunk, "format" => "sln"));
    ...
    if ($http_code == 200 && $audio_data) {
        // Save to local cache (PERSISTENT directly in SLN format)
        file_put_contents("$wav_file.sln", $audio_data);
        
        agi_log("Generated and cached SLN: $hash");
        return $hash;
    } else {

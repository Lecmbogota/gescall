...
router.get('/', async (req, res) => {
    try {
        const query = 'SELECT u.user_id, u.username, u.role_id, r.role_name as role, u.active, u.created_at, u.api_token FROM gescall_users u LEFT JOIN gescall_roles r ON u.role_id = r.role_id ORDER BY u.username ASC';
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const query = 'SELECT u.user_id, u.username, u.role_id, r.role_name as role, u.active, u.created_at, u.api_token FROM gescall_users u LEFT JOIN gescall_roles r ON u.role_id = r.role_id WHERE u.user_id = $1';
...
router.post('/', async (req, res) => {
    try {
        ...
        const query = `
            INSERT INTO gescall_users (username, password_hash, role_id, active)
            VALUES ($1, $2, $3, $4)
            RETURNING user_id, username, role_id, active, created_at
        `;

        const values = [username, password_hash, role_id, active !== undefined ? active : true];

...
async function regeneratePjsipUsers() {
    try {
        const { rows } = await pg.query('SELECT sip_extension, sip_password FROM gescall_users WHERE active = true AND sip_extension IS NOT NULL');
        ...
        exec('asterisk -rx "pjsip reload"', (err) => {
            if (err) console.error('[Users API] Error reloading PJSIP:', err);
            else console.log('[Users API] PJSIP reloaded successfully.');
        });
        
    } catch (err) {
        console.error('[Users API] Error generating PJSIP users conf:', err);
    }
}
...
router.get('/', async (req, res) => {
    try {
        const query = 'SELECT u.user_id, u.username, u.role_id, r.role_name as role, u.active, u.created_at, u.api_token, u.sip_extension FROM gescall_users u LEFT JOIN gescall_roles r ON u.role_id = r.role_id ORDER BY u.username ASC';
...
router.post('/', async (req, res) => {
    try {
        ...
        if (roleRes.rows.length > 0 && roleRes.rows[0].role_name.toUpperCase() === 'AGENT') {
            ...
            sipPassword = crypto.randomBytes(6).toString('hex');
        }

        const query = `
            INSERT INTO gescall_users (username, password_hash, role_id, active, sip_extension, sip_password)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING user_id, username, role_id, active, created_at, sip_extension
        `;

        const values = [username, password_hash, role_id, active !== undefined ? active : true, sipExtension?.toString() || null, sipPassword];
        const result = await pg.query(query, values);

        if (sipExtension) {
            await regeneratePjsipUsers();
        }
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (role_id !== undefined) {
            ...
            if (newRoleRes.rows.length > 0 && newRoleRes.rows[0].role_name.toUpperCase() === 'AGENT') {
                if (!existingSipExtension) {
                    const extRes = await pg.query("SELECT MAX(sip_extension::integer) as max_ext FROM gescall_users WHERE sip_extension ~ '^[0-9]+$'");
                    const newExtension = extRes.rows[0].max_ext ? parseInt(extRes.rows[0].max_ext) + 1 : 1000;
                    const sipPassword = crypto.randomBytes(6).toString('hex');

...
router.get('/', async (req, res) => {
    try {
        ...
        let allowedFilenames = null;

        // Filter by user group if not admin (level >= 8)
        const userLevel = parseInt(req.user?.user_level || '0', 10);
        if (userLevel < 8) {
            const userGroup = req.user?.user_group;
            if (userGroup) {
                // Get campaigns allowed for this group
                // Logic: Campaigns that have this user_group assigned
                const campaigns = await database.query(
                    'SELECT campaign_id FROM vicidial_campaigns WHERE user_group = ?',
                    [userGroup]
                );

                // Create Set of allowed filename prefixes: gc_campaignid.wav
                // We need to account for filename sanitization: lowercase and alphanumeric only
                const campaignIds = campaigns.map(c => c.campaign_id);
                allowedFilenames = new Set(campaignIds.map(id => {
                    const safeId = id.toLowerCase().replace(/[^a-z0-9]/g, '');
                    return `gc_${safeId}.wav`;
                }));
            } else {
                allowedFilenames = new Set(); // No group, no files
            }
        }
        ...
        for (const line of lines) {
            ...
            if (filename) {
                ...
                if (allowedFilenames && !allowedFilenames.has(filename)) {
                    continue;
                }
...
router.post('/upload', upload.single('audio'), async (req, res) => {
    try {
        ...
        try {
            ...
            // Automatically update campaign playback settings
            try {
                ...
                `, [campaign, finalFilename]);

                console.log(`[Audio] Automatically linked ${finalFilename} to campaign ${campaign}`);
            } catch (dbError) {
                console.error('[Audio] Failed to auto-link campaign:', dbError);
                // Don't fail the request, just log it. The file is uploaded.
            }

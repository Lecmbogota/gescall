<?php
// gescall_dialer.php - Custom Dialer with Proper AMI Reading
header('Content-Type: application/json');

$pass = isset($_REQUEST['pass']) ? $_REQUEST['pass'] : '';
$phone = isset($_REQUEST['phone_number']) ? $_REQUEST['phone_number'] : '';
$agent_ext = '8300'; 

if ($pass !== 'TEcnologia2020') {
    die(json_encode(["success" => false, "error" => "Authentication failed"]));
}

if (empty($phone)) {
    die(json_encode(["success" => false, "error" => "Phone number required"]));
}

$timeout = 10;
$socket = @fsockopen("127.0.0.1", 5038, $errno, $errstr, $timeout);

if (!$socket) {
    die(json_encode(["success" => false, "error" => "AMI Connection Failed: $errstr"]));
}

function get_response($socket, $expect_key = null) {
    $buffer = "";
    $result = "";
    // Loop until we find the key line or empty buffer (timeout/end)
    // Simple approach: Read until blank line, check if it contains what we want. 
    // If not, read next packet.
    
    // Safety break
    $start = time();
    while (time() - $start < 5) {
        $line = fgets($socket, 4096);
        $buffer .= $line;
        
        if (trim($line) == "") {
             // End of a packet.
             if ($expect_key && stripos($buffer, $expect_key) !== false) {
                 return $buffer;
             }
             // If we just want ANY response?
             if (!$expect_key) return $buffer;
             
             // Otherwise, save this packet and continue? 
             // AMI sends events mingled. logic is hard without ActionID.
             // But for Login, immediate response is expected.
             // For Originate, immediate response is expected.
             // Let's just return the packet.
             return $buffer;
        }
    }
    return $buffer; // Timeout
}

// Better approach with ActionID
$actionid_login = uniqid("login_");
fputs($socket, "Action: Login\r\n");
fputs($socket, "Username: cron\r\n");
fputs($socket, "Secret: 1234\r\n");
fputs($socket, "ActionID: $actionid_login\r\n\r\n");

// Read until we find our ActionID or fail
$login_res = "";
$start = time();
while(time() - $start < 5) {
    $pkt = get_response($socket);
    if (stripos($pkt, $actionid_login) !== false) {
        $login_res = $pkt;
        break;
    }
}

if (stripos($login_res, "Success") === false && stripos($login_res, "Accepted") === false) {
     fclose($socket);
     die(json_encode(["success" => false, "error" => "AMI Login Failed", "ami_response" => $login_res]));
}

// Originate
$actionid_orig = uniqid("orig_");
fputs($socket, "Action: Originate\r\n");
fputs($socket, "Channel: Local/$agent_ext@default/n\r\n");
fputs($socket, "Exten: $phone\r\n");
fputs($socket, "Context: default\r\n");
fputs($socket, "Priority: 1\r\n");
fputs($socket, "CallerID: $phone\r\n");
fputs($socket, "Async: yes\r\n");
fputs($socket, "ActionID: $actionid_orig\r\n\r\n");

$originate_res = "";
$start = time();
while(time() - $start < 5) {
    $pkt = get_response($socket);
    if (stripos($pkt, $actionid_orig) !== false) {
        $originate_res = $pkt;
        break;
    }
}

// Logoff
fputs($socket, "Action: Logoff\r\n\r\n");
fclose($socket);

echo json_encode([
    "success" => (stripos($originate_res, "Success") !== false || stripos($originate_res, "Queued") !== false),
    "message" => "Originate attempted",
    "details" => [
        "channel" => "Local/$agent_ext@default/n",
        "exten" => $phone,
        "ami_response" => $originate_res
    ]
]);
?>
